...
describe('urlToConfig', () => {
	...
	it('parses empty querystring into default config', () => {
	...
	it('parses unrecognised path to default config', () => {
		...
		const config = urlToConfig(url);
		expect(config).toEqual({ view: 'feed', query: { q: '' } });
	});
});
...
describe('configToUrl', () => {
	...
	it('converts default config to empty querystring', () => {
		const url = configToUrl(defaultConfig);

		...
		event.Records.map(
			async ({ messageId: sqsMessageId, messageAttributes, body }) => {
				...
				if (hasExternalId) {
					...
					logger.error({
						message: `Failed to put object to S3 with key "${objectKey}" in bucket "${config.feedsBucket}" for message with sqsMessageId ${sqsMessageId}.`,

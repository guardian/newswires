...
export async function putItemToDb({
	...
	const presetCategories = computePresetCategories(categoryCodes);
	const ingestedAt = (lastModified ?? new Date()).toISOString();
	try {
		const result = await sql`
		INSERT INTO ${sql(DATABASE_TABLE_NAME)}
			(external_id, supplier, content, category_codes, s3_key, ingested_at, preset_categories, last_updated_at)
		VALUES (${externalId}, ${supplier}, ${content as never}, ${categoryCodes}, ${s3Key}, ${ingestedAt}, ${presetCategories}, now()) ON CONFLICT (external_id) DO NOTHING
		RETURNING id`;
